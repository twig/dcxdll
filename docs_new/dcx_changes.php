...
$CHANGES = array(
    "v1.3.0" => array(
		...
		'added a pbar property [i]$xdid().text[/i]',
		'added ability to set no text on a button using <a>/xdid -t</a>',
		'added [e]keydown[/e], [e]keyup[/e] and [e]return[/e] to richedit',
		'link font is now the system default GUI font',
		'fixed <a>/xdid -C +k</a> for box control not using the specified color',
		'added [e]return[/e] and [e]rclick[/e] event to richedit',
		'added <a>/xdid -c</a> to copy content to clipboard for edit and richedit ctrls',
		'added [s]owner[/s] style to OpenDialog, SaveDialog and ColorDialog',
		'changed <a>-C</a> to <a>-S</a> for selecting text ranges in edit and richedit ctrls (this would of solved the coloring bug on these controls too)',
	...
	'v1.2.7 and v1.2.8 reimplementation' => array(
		...
		'changed <a>$dcx(GetSystemColor)</a>, please view documentation on changes to syntax',
		'added <a>/xdid -o</a> to overwrite a line for listbox ctrl',
		'added a style to the list control [s]vsbar[/s] allowing it to be scrolled when overflowing',
		'added [e]keydown[/e] and [e]keyup[/e] events to edit ctrl',
		'added <a>$dcx(ColorDialog)</a>, slightly changed syntax',
		'implemented <a>$dcx(OpenDialog)</a> and <a>$dcx(SaveDialog)</a>',
		'added [i]selstart[/i], [i]selend[/i], [i]sel[/i], [i]seltext[/i] properties to edit ctrl',
		'added [i]selstart[/i], [i]selend[/i], [i]sel[/i], [i]seltext[/i] properties to richedit ctrl',
		'fixed a bug in <a>/xdid -d</a> for tab ctrl where no page would be selected after deleting current page',
		'fixed a problem with box and link ctrls where cursor command <a>/xdid -J</a> wouldn\'t apply',
		'implemented <a>/xdid -U</a> for removing focus from a control. (will not work if there is only one control on the dialog)',
		'added [r]nocut[/r], [r]nocopy[/r] and [r]nopaste[/r] return values to prevent [e]cut[/e], [e]copy[/e] and [e]paste[/e] events for edit ctrl',
		'richedit editable by default',
		'added a style [s]readonly[/s] to richedit',
		'added <a>/xdid -Z</a> for zooming richedit ctrls',
		'Please note that [e]selchange[/e] (richedit) and [e]dropfile[/e] events are missing.<br />
