...
public:
	...
	int hitTest(const LPPOINT lpt) const noexcept;
	bool getItemRect(const int iButton, LPRECT lprc) const noexcept;
	Dcx::CodeValue<RECT, bool> getItemRect(const int iButton) const noexcept;
	int getButtonCount() const noexcept;
	LRESULT setButtonWidth(const int cxMin, const int cxMax) noexcept;
	bool deleteButton(const int iButton) noexcept;
	bool setButtonInfo(const int idButton, const LPTBBUTTONINFO lpbi) noexcept;
	int getButtonInfo(const int idButton, LPTBBUTTONINFO lpbi) const noexcept;
