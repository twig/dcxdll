...
DCXML::DCXML(const char *filePath,const char *dialogMark,const char *dialogName)
{
	this->setDialogMark(dialogMark);
	...
	if (this->getDialog()) {
		...
		if (this->getDialogsElement())
		{
			 ...
			 * which is used as a recursion start point for this->parseDialog()
			 */
...
void DCXML::parseAttributes(TiXmlElement* element) {
    elem = element->Value();
    parentelem = parent->Value();
    parenttype = (temp = parent->Attribute("type")) ? temp : "panel";
    weigth = (temp = element->Attribute("weight")) ? temp : "1";
...
void DCXML::setStyle(TiXmlElement* style) {
...
void DCXML::parseStyle(int depth) { 
    if (depth > 2) return;
    depth++;
...
void DCXML::parseIcons(int depth) { 
    if (depth > 1) return;
    depth++;
...
void DCXML::parseItems(TiXmlElement* element,int depth,char *itemPath) { 
    int item = 0;
    int cell = 0;
 ...
 void DCXML::parseTemplate(int dialogDepth,const char *claPath,int passedid)
{
...
void DCXML::parseDialog(int depth,const char *claPath,int passedid,int ignoreParent) { 
    TiXmlElement* child = 0;
    int control = 0;
    g_claPath = 0;
    g_claPathx = 0;
...
int DCXML::parseId(TiXmlElement* idElement)
	{
