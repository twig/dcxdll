...
enum class DcxIconSizes
	: int
{
	SmallIcon = 16, MediumIcon = 24, LargeIcon = 32, FortyEight = 48, SixtyFour = 64, OneTwoEight = 128, TwoFiveSix = 256,
	MaxSize = TwoFiveSix
};
...
struct ALPHAINFO
{
	...
	ALPHAINFO(const HDC& ai_hdc, HDC* ai_hdcBuffer, const HDC& ai_Oldhdc, const HBITMAP& ai_bitmap, const HBITMAP& ai_bkg, const HBITMAP& ai_oldBM, const RECT& ai_rcClient, const RECT& ai_rcWin, const BLENDFUNCTION& ai_bf, const HPAINTBUFFER& ai_Buffer, const HPAINTBUFFER& ai_BufferAlpha, const HDC& ai_hdcAlpha) noexcept
